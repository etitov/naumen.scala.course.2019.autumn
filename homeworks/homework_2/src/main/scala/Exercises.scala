    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
        ...
        while (unsortedBalls.nonEmpty){
            var lightBallWeight = Double.MaxValue
            var lightBallName = ""
            for (ball <- unsortedBalls){
                val currentBallWeight = 4 / 3 * Math.PI * Math.pow(ballsArray(ball)._1, 3) * ballsArray(ball)._2
                if (currentBallWeight < lightBallWeight) {
                    lightBallName = ball
                    lightBallWeight = currentBallWeight
                }
            }
