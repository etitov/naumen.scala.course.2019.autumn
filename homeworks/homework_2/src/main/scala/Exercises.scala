...
object Exercises {
  ...
  class PrimeGenerator {
    private var primes = scala.collection.mutable.Seq.empty[Int]
  ...
  def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D,
    ...
    val first = function(leftVec0, leftVec1)
    val second = function(rightVec0, rightVec1)
    return first + second
  }
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = {
    val massBalls = ballsArray.map(x => (x._1, (4 / 3 * Math.PI * Math.pow(x._2._1, 3)) * x._2._2))
    val sortedMassBalls = massBalls.toSeq.sortBy(_._2)
    return sortedMassBalls.map(y => y._1)
  }
