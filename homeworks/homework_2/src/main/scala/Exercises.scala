...
object Exercises {
  ...
  на 3 или на 5.*/
  /*Реализовать юнит-тесты в src/test/scala для данной функции.*/
  def sumOfDivBy3Or5(iFrom: Int, iTo: Int): Long = (iFrom to iTo).filter(i => i % 3 == 0 || i % 5 == 0).toArray.sum
  ...
  def primeFactor(number: Int): Seq[Int] = {
    val primeFactors = ListBuffer[Int]()
    var rest = number

    for (i <- (2 to rest).filter(n => isFactor(n, rest))) {
      if (isPrime(i)) {
        primeFactors.append(i)
        rest = rest / i;
      }
    }
    primeFactors.distinct.toSeq
  }

  def isFactor(number: Int, forNumber: Int): Boolean = forNumber % number == 0

  def isPrime(number: Int): Boolean = (2 until number).forall(d => number % d != 0)
  ...
  def sumByFunc(leftVec0: Vector2D, leftVec1: Vector2D, functor: (Vector2D, Vector2D) => Double, rightVec0: Vector2D, rightVec1: Vector2D): Double =
   functor(leftVec0, leftVec1) + functor(rightVec0, rightVec1)
  ...
  */
  ...
  def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] = (ballsArray map {
    case (metal, (radius, density)) => (metal, getWeight(radius, density))
  }).toSeq
    .sortBy(e => e._2)
    .map(e => e._1)

  def getWeight(radius: Int, density: Double): Double = getVolume(radius) * density

  def getVolume(radius: Int): Double = (4 * java.lang.Math.PI * java.lang.Math.pow(radius, 3)) / 3
